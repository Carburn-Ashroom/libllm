...
namespace LLM_impl {             // 该名字空间负责实现大模型的基类
    ...
    protected:
        ...
        void set_call_back(function<size_t(void*, size_t, size_t, Message_func*)> func) { call_back_func = func; }
    ...
    private:
        ...
        double temperature;
        function<size_t(void*, size_t, size_t, Message_func*)> call_back_func;
    };
    ...
    public:
        ...
        void get(string&& question) override             // 调用大模型
        {
            ...
            curl.set_write_func(reinterpret_cast<void*>(*(func_call_back().target<size_t(*)(void*, size_t, size_t, Message_func*)>())));
    ...
    private:
        function<void(string&&, bool)> func;
        string last_reason;
        static size_t call_back(void* contents, size_t size, size_t nmemb, Message_func* ptr)           // 处理网络请求中每次返回的json
        {
            if (not contents)
                throw LLM_error{"服务器繁忙，请稍后再试。"};
            string json {encode(CP_UTF8, ptr->prog_enc(), reinterpret_cast<char*>(contents))};
